...
export class CreateReportDto {
    ...
    @IsNotEmpty({ message: 'bookingId is required for check-in and check-out reports' })
    @IsString()
    bookingId?: string;
    ...
    @IsString()
    @IsEnum(ReportType, { message: 'type must be one of values (checkIn, checkOut, or maintenance)' })
    type!: ReportType;

    @IsNotEmpty({ message: 'severity is required' })
    @IsString()
    @IsEnum(ReportSeverity, { message: 'severity must be one of values (Low, Medium, High)' })
...
export class GetStaffReportsQuery {
    ...
    @IsOptional()
    @IsEnum(ReportStatus, { message: 'status must be one of values (Pending, Approved, Rejected)' })
    status?: ReportStatus;

    @IsOptional()
    @IsEnum(ReportType, { message: 'type must be one of values (checkIn, checkOut, maintenance)' })
    type?: ReportType;
