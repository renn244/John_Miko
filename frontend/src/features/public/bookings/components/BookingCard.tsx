...
import type { BookingWithAccommodationAndFeedback, BookingWithAccommodationAndPreOrder } from "@/features/shared/bookings/types/booking.type";
                        ...
                        {isDetailLoading ? (
                            ...
                            <>
                                ...
                                <div className="mt-4 space-y-2 rounded-lg border bg-background p-3 text-sm">
                                    ...
                                    {booking.status !== 'Cancelled' && payment.status !== 'Refunded' && <div className="flex items-center justify-between gap-3">
                                ...
                                {payment.rejectionNote && (
                                    <div className="mt-4 text-sm">
                                        <h5 className="font-semibold">Payment rejection reason</h5>
                                        <p className="mt-1 whitespace-pre-wrap break-words text-muted-foreground">{payment.rejectionNote}</p>
                                    </div>
                                )}
                                {payment.status === 'Refunded' && (
                                    <section className="mt-4 space-y-3 rounded-lg border bg-background p-3 text-sm" aria-label="Refund details">
                                        <h5 className="font-semibold">Refund recorded</h5>
                                        <div className="flex justify-between gap-3">
                                            <span className="text-muted-foreground">Refund amount</span>
                                            <span className="font-semibold">{formatPeso(payment.amountPaid)}</span>
                                        </div>
                                        <p className="text-muted-foreground">
                                            Recorded: {payment.refundedAt ? new Date(payment.refundedAt).toLocaleString('en-PH', { timeZone: 'Asia/Manila', dateStyle: 'medium', timeStyle: 'short' }) : 'Date unavailable'}
                                        </p>
                                        <div>
                                            <h6 className="font-medium">Refund reason</h6>
                                            <p className="mt-1 whitespace-pre-wrap break-words text-muted-foreground">{payment.refundReason || 'Reason unavailable'}</p>
                                        </div>
                                        {payment.refundProofImageUrl ? (
                                            <ViewPhotoDialog imageUrl={payment.refundProofImageUrl} imgOptions={{ alt: 'Recorded refund proof' }}>
                                                <Button type="button" variant="outline" size="sm">View refund proof</Button>
                                            </ViewPhotoDialog>
                                        ) : <p className="text-muted-foreground">Proof unavailable</p>}
                                    </section>
                                )}
                            </>
                        ) : (
