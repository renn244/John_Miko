...
export const knowledgeApi = {
  getKnowledgeDocuments: async (query: KnowledgeDocumentsQuery) => {
    ...
    if (response.status >= 400) {
      throw new Error(response.data.message || "Unable to load knowledge documents.");
    }
  ...
  getKnowledgeDocumentById: async (id: string) => {
    ...
    if (response.status >= 400) {
      throw new Error(response.data.message || "Unable to load this document.");
    }
  ...
  createKnowledgeDocument: async (data: KnowledgeDocumentInput) => {
    ...
    if (response.status >= 400) {
      throw new Error(response.data.message || "Unable to create the document.");
    }

    return response.data as KnowledgeDocument;
  },
  updateKnowledgeDocument: async (id: string, data: Partial<KnowledgeDocumentInput>) => {
    const response = await apiClient.patch(`/knowledge/${id}`, data);

    if (response.status >= 400) {
      throw new Error(response.data.message || "Unable to save the document.");
    }
  ...
  publishKnowledgeDocument: async (id: string) => {
    ...
    if (response.status >= 400) {
      throw new Error(response.data.message || "Unable to publish the document.");
    }
  ...
  unpublishKnowledgeDocument: async (id: string) => {
    ...
    if (response.status >= 400) {
      throw new Error(response.data.message || "Unable to unpublish the document.");
    }
  ...
  deleteKnowledgeDocument: async (id: string) => {
    ...
    if (response.status >= 400) {
      throw new Error(response.data.message || "Unable to delete the document.");
    }
