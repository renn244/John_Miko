...
import { useApprovePaymentMutation, useRefundPaymentMutation, useRejectPaymentMutation } from "@/features/admin/payments/hooks/useAdminPayments";
...
const BookingViewContent = ({ booking }: { booking: BookingWithAccommodationAndPreOrderAndPayment }) => {
    ...
    const canRecordCancelledBookingRefund = payment?.status === "Approved" && booking.status === "Cancelled";
            ...
            <AdminPageHeader
                ...
                actions={
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant="secondary">{booking.source ?? "Online"}</Badge>
                        <Badge
                            className="border"
                            style={{
                            ...
                            <div className="space-y-4">
                                ...
                                {payment.status === "Rejected" && (
                                    <div className="space-y-3 rounded-lg border border-violet-200 bg-violet-50 p-3 text-sm text-violet-950">
                                        <div>
                                            <p className="font-semibold text-violet-800">Was money received from the guest?</p>
                                            <p className="mt-1 text-violet-900/80">
                                                If it was received and returned, record the refund and attach its receipt or screenshot. For a troll or invalid payment that was never received, no refund record is needed.
                                            </p>
                                        </div>
                                        <Button
                                            type="button"
                                ...
                                {canRecordCancelledBookingRefund && (
                                    <div className="space-y-2 border-t pt-4">
                                        <p className="text-sm text-muted-foreground">
                                            This cancelled booking has an approved payment. Record the return of {formatPeso(payment.amountPaid)} to the guest.
                                        </p>
                                        <Button
                                            type="button"
                                            variant="destructive"
