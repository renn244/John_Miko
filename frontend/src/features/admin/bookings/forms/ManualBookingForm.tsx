...
import { useCreateBookingAdminMutation, useCreateWalkInBookingAdminMutation } from "@/features/admin/bookings/hooks/useAdminBookings";
...
const ManualBookingForm = ({ mode = "manual" }: { mode?: "manual" | "walkIn" }) => {
    ...
    const form = useForm<ManualBookingFormValues>({
        ...
        defaultValues: {
            ...
            proofImageUrl: "",
            accommodationId: "",
            checkIn: isWalkIn ? new Date(toDateOnly(new Date())) : undefined,
            stayOptionId: undefined,
            addOnServices: [],
            preOrderItems: [],
            paymentType: isWalkIn ? "Full" : undefined,
        },
        criteriaMode: "all"
    });
    const {
    ...
    const manualBooking = useCreateBookingAdminMutation();
    const walkInBooking = useCreateWalkInBookingAdminMutation();
    const isPending = isWalkIn ? walkInBooking.isPending : manualBooking.isPending;
    ...
    const onSubmit = async (data: ManualBookingFormValues) => {
        ...
        try {
            ...
            const booking = isWalkIn
                ? await walkInBooking.mutateAsync(bookingBase)
                : await manualBooking.mutateAsync({ ...bookingBase, paymentType: data.paymentType });

            toast.success(isWalkIn ? "Walk-in recorded successfully" : "Booking created successfully");
                ...
                <FormSection title="Stay Details" contentClassName="space-y-5">
                        ...
                        render={({ field, fieldState }) => (
                            <Field data-invalid={fieldState.invalid} className="grid gap-2">
                                ...
                                value={field.value}
                                onValueChange={(value) => {
                                    if (!isWalkIn) resetField('checkIn')
                        ...
                        <div className="grid gap-5">
                            ...
                            render={({ field, fieldState }) => (
                                <Field data-invalid={fieldState.invalid} className="grid gap-2">
                                    ...
                                    {isWalkIn ? (
                                        <div className="flex h-9 w-70 items-center rounded-md border bg-muted px-3 text-sm font-medium">
                                            {format(new Date(field.value), "PPP")} (today)
                                        </div>
                                    ) : <Popover>
                        ...
                        {isAccommodationSelected && (
                            <div>
                                <div className="space-y-2 text-sm">
                                    ...
                                    {!isWalkIn && <div className="flex justify-between items-center">
                                        <span className="text-muted-foreground">Food Preorders:</span>
                                        <span className="font-semibold">₱{preOrderSubtotal.toLocaleString()}</span>
                                    </div>}
            ...
            <div className="rounded-xl border border-border bg-card px-5 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <Button type="submit" disabled={isPending}>
                        {isPending ? <LoadingSpinner /> : isWalkIn ? "Record Walk-in" : "Create Booking"}
                    </Button>
                </div>
            </div>
        </form>
